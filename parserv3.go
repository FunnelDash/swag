...
// copyPropSchemaV3 returns a shallow copy of a struct property's schema safe to
// decorate (e.g. stamp x-order) without mutating a shared/cached source. The
// schema struct and its Extensions map are copied; deeper nodes (Properties,
// Items, Enum) are shared, which is fine for the top-level decoration callers do.
